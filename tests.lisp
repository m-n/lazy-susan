...
(defun run-tests ()
  (let ((fails 0)
        failing-tests)
    (declare (special fails failing-tests))
    (mapcar (lambda (test)
...
(defmacro test-and (&body forms)
  (let ((f (gensym)))
    (cond ((null forms) t)
          (t `(let ((,f ,(car forms)))
                (declare (special fails current-test failing-tests))
                (test-and ,@(cdr forms))
                (if  (not ,f)
  ...
  `(progn (pushnew ',name *tests*)
          (defun ,name ()
            (let ((current-test ',name))
              (declare (special fails current-test failint-tests))
              (handler-case (test-and ,@body)
                (error (c) (incf fails) (format t "Failed; threw ~S~&" c)))))))
